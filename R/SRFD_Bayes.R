...
SRFD_Bayes <- function(train_data, test_data, method="SVM") {
  ...
  if(param@oversample){ # not yet tested
    os <- borderline_smote(x$R, param@train_gt_label, 40, "fraction", param)
    reconstruction_err <- train_data - x$W %*% x$R
    os_data <- mix_WH(x$W, os$synthetic_data, os$synthetic_labels, reconstruction_err, param)
    train_data <- cbind(train_data, os_data)
    param@train_gt_label <<- c(param@train_gt_label, os$synthetic_labels)
    #param@train_gt_label <- c(param@train_gt_label, os$synthetic_labels)
    sorted_index <- sort.list(param@train_gt_label)
  ...
  cat("\n2.1 Estimating training data source fractions\n")
  train_sf <- deconvolution_with_reference(train_data, x$W, rep(0, ncol(train_data)), n.tries=3)
  cat("\n2.2 Estimating testing data source fractions\n")
  ...
  for (j in 1:10) {
    cat(".")
    bd <- Bayes_diagnosis(x = train_data, y = param@train_gt_label, newx = test_data, train_sf, test_sf, method=tolower(method), param=param)
  ...
  suppressWarnings(rm(train_data, test_data, train_sf, test_sf, train_labels, test_labels, train_class_index, test_class_index, sorted_train_index, sorted_test_index, bayes_pred_prob_all, bd))
  gc()
